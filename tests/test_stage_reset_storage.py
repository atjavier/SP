...
class StageResetStorageTestCase(unittest.TestCase):
    ...
    def test_reset_stage_and_downstream_clears_dbsnp_evidence_when_resetting_annotation(self):
        from storage.db import init_schema, open_connection  # noqa: E402
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertEqual(len(list_dbsnp_evidence_for_run(db_path, run_id, limit=10)), 1)

            reset_stage_and_downstream(db_path, run_id, "annotation")

            self.assertEqual(list_dbsnp_evidence_for_run(db_path, run_id, limit=10), [])

    def test_reset_stage_and_downstream_clears_clinvar_evidence_when_resetting_annotation(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertEqual(len(list_clinvar_evidence_for_run(db_path, run_id, limit=10)), 1)
